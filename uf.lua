...
if mechMod then
    local ok, result = pcall(require, mechMod)
    if ok then mechMod = result else mechMod = nil end
end
-- ============================================================
-- STATE VARIABLES
-- ============================================================
...
local function getFootball()
    ...
    -- fallback: brute search entire workspace
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("BasePart") and (obj.Name == "Football" or obj.Name == "918f5408-d86a-4fb8-a88c-5cab57410acf") then
            cachedBall = obj
            return obj
        end
    end
...
local function getFootball()
	...
	for _, obj in ipairs(Workspace:GetDescendants()) do
		if obj:IsA("BasePart") and obj.Name == "Football" then
			cachedBall = obj; return cachedBall
		end
	end
end

Workspace.DescendantAdded:Connect(function(obj)
	if obj:IsA("BasePart") and obj.Name == "Football" then cachedBall = obj end
end)
Workspace.DescendantRemoving:Connect(function(obj)
	if obj == cachedBall then cachedBall = nil end
end)

local function pullToBall(hrp)
	local ball = getFootball()
	if not ball or not hrp then return end
	local velocity = ball.Velocity
	local offset = (velocity.Magnitude > 0) and (velocity.Unit * 15) or Vector3.zero
	local targetPos = ball.Position + offset + Vector3.new(0, 3, 0)
	local lookDir = (ball.Position - hrp.Position).Unit
	hrp.CFrame = hrp.CFrame:Lerp(
		CFrame.new(targetPos, targetPos + lookDir),
		magnetSmoothness
	)
end
local pullJumpConnection

local function bindCharacter(char)
	local hrp = char:WaitForChild("HumanoidRootPart")
	local humanoid = char:WaitForChild("Humanoid")
	if pullJumpConnection then pullJumpConnection:Disconnect() end
	pullJumpConnection = humanoid.Jumping:Connect(function()
		if PULL_ENABLED then pullToBall(hrp) end
	end)
end
...
UserInputService.InputBegan:Connect(function(input, gp)
   ...
   if input.UserInputType == Enum.UserInputType.Keyboard then
    ...
    local toggle = Fluent.Options["JumpBoostToggle"]
    if toggle then toggle:SetValue(jumpBoostActive) end
end

    if input.UserInputType == Enum.UserInputType.Gamepad1
    and input.KeyCode == Enum.KeyCode.ButtonL1 then
        enabled = not enabled
        local toggle = Fluent.Options["StickyToggle"]
        if toggle then toggle:SetValue(enabled) end
    end
...
Tabs.Main:AddToggle("PullToggle", {
	Title    = "jump pull vector",
	Default  = PULL_DISABLED,
	Callback = function(v)
		PULL_ENABLED = v
	end,
})

Tabs.Main:AddInput("SmoothnessInput", {
	Title = "Smoothness",
	Default = "0.048",
...
-- ============================================================
-- GRAVITY (Catching Tab)
-- ============================================================
local gravityEnabled = false
local customGravity  = 50
local defaultGravity = Workspace.Gravity

Tabs.Main:AddParagraph({ Title = "Gravity", Content = "Lower = jump higher, vice versa." })
Tabs.Main:AddToggle("GravityToggle", {
    Title   = "Low Gravity",
    Default = false,
    Callback = function(v)
        gravityEnabled = v
        Workspace.Gravity = v and customGravity or defaultGravity
    end
})
Tabs.Main:AddInput("GravityInput", {
    Title       = "Gravity Value",
    Description = "Default is 196.2.",
    Default     = "50",
    ...
    Callback    = function(v)
        ...
        if num then
            customGravity = num
            if gravityEnabled then Workspace.Gravity = num end
        end
    end
})
plr.CharacterAdded:Connect(function(char)
    if jumpBoostEnabled then setupJumpBoost(char) end
    if gravityEnabled then Workspace.Gravity = customGravity end  -- add this
end)
...
Tabs.Player:AddParagraph({ Title = "Jump Boost", Content = "Launches you upward when touching opponents. Press F or R1 to toggle on/off." })
...
Tabs.Player:AddSlider("BallDetectionRadius", { Title="Ball Detection Radius", Default=10, Min=5, Max=100, Rounding=0, Callback=function(v) BALL_DETECTION_RADIUS=v end })
...
Tabs.Hitbox:AddToggle("StickyToggle", {
    Title = "Sticky Head",
    Description = "Press R or L1 to toggle. When enabled, your character will be pulled toward the heads of nearby opponents for easier catching.",
    Default = false,
...
local emoteList = {
    {"Bop",8028669437},
	{"BlocBoy Shoot",2178463446},
	{"Hacker's Delight",10714364213},
	{"Ball Spin",14215798544},
    {"Head dribble",11006158861},
	{"The griddy",8028694339},
	{"Penguin Dance",5439075558},
	{"Goopie",5439090599},
    {"Headless",5704065738},
	{"Worm",3471311681},
	{"Speed dance",94471347736309},
	{"Peanut Butter Jelly Time",5433555683},
    {"Moon",14216002323},
	{"Inner child",14215788817},
	{"Neighborly hang",11006145037},
	{"Headspin",14920821886},
    {"Take the L",2293391158},
	{"Couldnt care less",107875941017127},
	{"Memphis pregame",14138482621},
	{"Rocket landing",103979887627824},
	{"Smith Knicks",15312473847},
	{"Dimension",5618747341},
	{"Eagle Flap",2293380203},
	{"Keep it hot",85267023718407},
	{"Sturdy",14215791622},
	{"Samurai Slash",15249657798},
	{"Thug Shake",13550466835},
	{"Calamity",3471359323},
	{"Dizzy",10714066964},
	{"Turbulence",13643188123},
	{"Druski Dance",14901504235},
	{"Scuba",140349022227594},
	{"Reanimated",2293388847},
	{"Crackdown",8028701898},
	{"Injured Griddy",14598292070},
	{"Anotha One Sway",77780134372141},
	{"Flag Wave",14598295080},
	{"Robics",3471306837},
}
